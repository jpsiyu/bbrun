...
class Map {
    ...
    resize() {
        this.setMapCfg()
        this.gridSize = this.calGridSize(this.context.canvas.width, this.mapCfg.gridInRow)
        tool.resizeCanvas(
            GameGlobal.g.screenContext.canvas.width,
            GameGlobal.g.context.canvas,
            this.mapCfg.gridInRow
        )
    }

    setMapCfg() {
        const lv = GameGlobal.g.gameLv
        let curCfg
        let set = false
